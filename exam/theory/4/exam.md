...
Граф конденсации -- граф, получаемый из данного графа сжатием каждой компоненты связности в одну вершину. Каждой вершине
графа конденсации соответствует компонента сильной связности графа G, а ориентированное ребро между вершинами C_i, C_j
графа конденсации проводится, если найдется пара вершин u из C_i и v из C_j, между которыми существовало ребро в
исходном графе
...
Предположим, что из C можно прийти в С'. Докажем, что из C' нельзя прийти в C.

Из определения конденсации получаем, что найдутся две вершины u из C и v из C', что из u можно прийти в v.

От противного: предположим, что из C' можно прийти в C. Тогда найдутся две вершины u' из С и v' из C', что из v' можно
прийти в v' в u'.

Т.к u, u' находятся в одной компоненте сильной связности, то между ними есть путь (аналогично v, v').

Объединяем пути и получаем, что v -> u, u -> v. Следовательно, u, v лежат в одной компоненте сильной связности.

Противоречие, ацикличность доказана.